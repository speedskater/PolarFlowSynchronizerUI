...
!define exec "polar-flow-synchronizer-ui.exe"
...
Section "Install"
    ...
    CreateShortCut "$DESKTOP\${MUI_PRODUCT}.lnk" "$INSTDIR\${exec}" ""

    ; Create start menu shortcut
    CreateDirectory "$SMPROGRAMS\${MUI_PRODUCT}"
    CreateShortCut "$SMPROGRAMS\${MUI_PRODUCT}\Uninstall.lnk" "$INSTDIR\${uninstaller}" "" "$INSTDIR\${uninstaller}" 0
    CreateShortCut "$SMPROGRAMS\${MUI_PRODUCT}\${name}.lnk" "$INSTDIR\${exec}" "" "$INSTDIR\icon.ico"
...
Section "Uninstall"
    ...
    RMDir /r "$SMPROGRAMS\Polar Flow Synchronizer\"
